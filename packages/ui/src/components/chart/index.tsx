  ...
  (
    ...
    return (
      <div
        ...
        className={cn(
          floatingPanelClass,
          "grid min-w-[8rem] items-start gap-1.5 px-2.5 py-1.5 text-xs dark:border-neutral-800 dark:bg-neutral-950 dark:text-neutral-50 dark:shadow-none",
          className,
        )}
      >
                      ...
                      {itemConfig?.icon ? (
                        ...
                        !hideIndicator && (
                          <div
                            ...
                            style={indicatorStyle(indicatorColor)}
                          />
                        )
                      )}
                      <div
...
// SAFETY: React types `style` from csstype, which has no room for custom properties.
const indicatorStyle = (color: string | undefined) =>
  ({
    "--color-bg": color,
    "--color-border": color,
...
function readString(source: unknown, key: string) {
  ...
  const [, value] = Object.entries(source).find(([name]) => name === key) ?? [];
...
function getPayloadConfigFromPayload(
  ...
  const nested = "payload" in payload ? payload.payload : undefined;

  const configLabelKey =
    readString(payload, key) ?? readString(nested, key) ?? key;
