#!/usr/bin/env python3
"""Improve a skill description from eval results (CI / unattended automation).

In IDE sessions, use agents/improve-description.md via a subagent instead.
"""
...
from scripts.platform import AgentBackend, call_agent_text
...
def improve_description(
    ...
    """Ask an agent to improve the description based on eval results."""
    failed_triggers = [
...
Please respond with only the new description text in <new_description> tags, nothing else."""
    ...
    text = call_agent_text(prompt, model, backend)
    ...
    if len(description) > 1024:
        ...
        shorten_text = call_agent_text(shorten_prompt, model, backend)
...
def main():
    ...
    parser.add_argument(
        "--backend",
        default="auto",
        choices=["auto", "claude", "cursor-sdk"],
        help="Agent backend for description improvement (default: auto-detect)",
    )
